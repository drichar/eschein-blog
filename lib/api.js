...
function extractContent(fetchResponse) {
  return fetchResponse?.data?.contentModelCollection?.items?.[0]
}

export async function getContentById(id) {
  const entry = await fetchGraphQL(
    `query {
      contentModelCollection(where: { id: "${id}" }, limit: 1) {
        items {
